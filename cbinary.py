...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Convert an ascii sequence into binary or inverse')
    ...
    if args.ascii is not None:
        print('Convertion of "{}" into binary sequence'.format(args.ascii))
        bits = text_to_bits(args.ascii)
        print(' '.join([bits[i:i+8] for i in range(0, len(bits), 8)]))
    if args.binary is not None:
        print('Convertion the binary into ASCII sequence'.format(args.binary ))
